...
The Firebase probe validates the configured Admin SDK credential/project, performs a one-user list permission/connectivity request without returning user data, and reads the Firebase Auth project policy to confirm Google sign-in is enabled, Phone Auth is enabled, and India (`IN`) is present in the SMS-region allowlist. This policy check is read-only and exists because an enabled Phone provider with an empty SMS allowlist still causes customer OTP requests to fail. If Razorpay is intentionally disabled, its probe reports `skipped` and the combined canary may still pass. If Razorpay is requested, its probe performs only `GET /v1/orders?count=1` using verified live credentials and returns no order details; in that state it must pass for exit code `0`.
