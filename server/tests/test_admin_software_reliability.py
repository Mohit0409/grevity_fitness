...
class AdminSoftwareReliabilityTests(unittest.TestCase):
    ...
    def test_plan_filter_is_applied_before_the_customer_page_limit(self) -> None:
        """Known regression: the current list limit precedes the plan filter.

        Keep this as an expected failure until Chat 1 moves the membership filter
        into the SQL query (or pages after filtering). The test prevents the
        correctness issue from being lost in a performance-only discussion.
        """
        rows = [
            (
