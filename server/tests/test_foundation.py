...
class DatabaseTests(unittest.TestCase):
    ...
    def test_migrations_are_idempotent_and_database_is_healthy(self):
        with TemporaryDirectory() as temporary:
            ...
            self.assertEqual(database.migrate(), ["001", "002", "003", "004", "005", "006", "007", "008", "009", "010"])
            self.assertEqual(database.migrate(), [])
            self.assertEqual(database.health(), {"database": "ok", "migrations": "10"})
            with closing(sqlite3.connect(path)) as connection:
                versions = connection.execute("SELECT version FROM schema_migrations").fetchall()
                self.assertEqual(versions, [("001",), ("002",), ("003",), ("004",), ("005",), ("006",), ("007",), ("008",), ("009",), ("010",)])
...
class HttpFoundationTests(unittest.TestCase):
    ...
    def test_security_headers_are_present_without_server_version_leak(self):
        with running_server() as (base, _settings):
            ...
            self.assertIn("https://apis.google.com", directives["script-src"])
            self.assertIn("https://accounts.google.com", directives["frame-src"])
            self.assertIn("https://recaptcha.google.com", directives["frame-src"])
            self.assertIn("https://www.googleapis.com", directives["connect-src"])
            self.assertIn("https://www.google.com", directives["connect-src"])
            self.assertIn("https://www.recaptcha.net", directives["connect-src"])
            self.assertNotIn("googletagmanager.com", csp)
